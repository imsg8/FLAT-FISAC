...
flowchart LR
  subgraph GRU
    ...
    ResetGateCalc[Calculate r_t = \sigma(W_r \cdot [h_{t-1}, x_t])]
    ResetGate --> ResetGateCalc

    UpdateGate[Update Gate]
    UpdateGateCalc[Calculate z_t = \sigma(W_z \cdot [h_{t-1}, x_t])]
    UpdateGate --> UpdateGateCalc

    CandidateActivation[Candidate Activation]
    CandidateActivationCalc[Calculate \tilde{h}_t = \tanh(W \cdot [r_t * h_{t-1}, x_t])]
